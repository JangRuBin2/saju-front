...
| 영역 | 상태 | 비고 |
|------|------|------|
| 페이지 (11+) | ✅ 완료 | 홈, 로그인, 사주, 운세, 궁합, 결제, 마이페이지 |
| 인증 (Auth.js v5) | ✅ 완료 | 카카오/네이버/구글 OAuth 코드 완성 |
| DB 스키마 (Prisma) | ✅ 완료 | 7개 테이블 설계 완료 |
| BFF API 레이어 | ✅ 완료 | Server Actions + SSE 프록시 |
| 결제 (토스페이먼츠) | ✅ 완료 | SDK 연동, 주문/확인/웹훅 |
| 사용량 제한 | ✅ 완료 | 무료/프리미엄 tier 분기 |
| i18n (4개 언어) | ✅ 완료 | ko, en, ja, zh |
...
### UI-01: 에러 핸들링 UI 보강
- **상태**: ❌ 미완료
- **설명**: `ErrorMessage` 컴포넌트가 존재하지만 어디서도 사용되지 않음
- **파일**: `src/components/ui/ErrorMessage.tsx`
- **작업**:
  - [ ] 사주 결과 페이지 에러 표시 추가
  - [ ] 운세/궁합 페이지 에러 표시 추가
  - [ ] API 타임아웃/네트워크 에러 사용자 안내
  - [ ] OAuth 로그인 실패 에러 표시 (`/login` 페이지)

### UI-02: 로딩 상태 개선
- **상태**: 🔶 부분 구현
- **설명**: 일부 비동기 작업에 로딩 인디케이터 미표시
- **작업**:
  - [ ] 사주 계산 중 로딩 애니메이션 (LoadingBook 활용)
  - [ ] 결제 처리 중 로딩 상태
  - [ ] 프로필 저장 중 버튼 비활성화

### UI-03: 폼 유효성 검증 강화
- **상태**: 🔶 기본만 구현
- **파일**: `src/components/saju/SajuForm.tsx`
- **작업**:
  - [ ] 생년월일 유효 범위 검증 (윤달, 존재하지 않는 날짜 등)
  - [ ] 출생 연도 상한값 동적화 (현재 2030 하드코딩)
  - [ ] 궁합 폼 — 두 사람 동일 정보 입력 시 경고
  - [ ] 프로필 저장 시 입력값 유효성 검증
...
- **상태**: ❌ 미개발
- **설명**: 로그인 후 사용자에게 종합 정보를 보여주는 대시보드
- **위치**: `src/app/[locale]/dashboard/page.tsx` (신규)
- **기능**:
  - [ ] 오늘의 운세 요약 카드
  - [ ] 최근 사주 분석 이력
  - [ ] 사용량 현황 (무료 N/1회 사용, 프리미엄 무제한)
  - [ ] 빠른 액세스 메뉴 (사주, 궁합, 운세)
  - [ ] 프리미엄 미가입 시 업그레이드 배너
...
- **상태**: ❌ 버그
- **파일**: `src/app/api/payments/confirm/route.ts`
- **설명**: `/ko/` 하드코딩 → 사용자 현재 로케일 반영
- **작업**:
  - [ ] 결제 요청 시 locale 파라미터 전달
  - [ ] 리다이렉트 URL에 올바른 locale 사용
...
- **작업**:
  - [ ] OAuth 콜백 에러 파라미터 처리 (`?error=...`)
  - [ ] 에러 종류별 사용자 안내 메시지
  - [ ] 재시도 버튼

### AUTH-02: 로그인 후 리다이렉트 (callbackUrl)
- **상태**: ❌ 미완료
- **설명**: 보호된 페이지 접근 시 → 로그인 → 원래 페이지로 복귀
- **작업**:
  - [ ] 미들웨어에서 `callbackUrl` 쿼리 파라미터 전달
  - [ ] 로그인 성공 후 callbackUrl로 리다이렉트
...
- **작업**:
  - [ ] 계정 삭제 확인 UI
  - [ ] 관련 데이터 cascade 삭제 Server Action
  - [ ] 삭제 후 세션 정리 및 로그아웃
...
| 순위 | Phase | 핵심 작업 | 이유 |
|------|-------|----------|------|
| 1 | 🔴 Phase 0 | 인프라 셋업 | 앱 실행 자체가 불가 |
| 2 | 🟡 Phase 1 | API 연동 테스트 | 핵심 기능 검증 |
| 3 | 🟡 Phase 2 | UI 개선 | 사용자 경험 |
| 4 | 🟡 Phase 4 | 결제 버그 수정 | 매출 직결 |
| 5 | 🟡 Phase 5 | 인증/보안 | 서비스 안정성 |
| 6 | 🟡 Phase 3 | 대시보드 | 신규 기능 |
| 7 | 🟢 Phase 6 | 배포 | 서비스 런칭 |
| 8 | 🔵 Phase 7 | 추가 기능 | 부가 가치 |
